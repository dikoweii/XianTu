...
## 核心规则
**状态效果可以修改，但不能直接删除！删除由系统根据时间自动处理。**

## 允许的操作

### ✅ 添加新状态效果
\`\`\`json
{"action": "push", "key": "玩家角色状态.状态效果", "value": {
  "状态名称": "重伤",
  "描述": "身受重伤，行动不便",
  "强度": -10,
  "持续时间分钟": 120,
...
### ✅ 修改现有状态效果
可以修改状态效果的任何字段（描述、强度、持续时间等）

**示例：服用疗伤丹后伤势好转**
\`\`\`json
{"action": "set", "key": "玩家角色状态.状态效果[0].状态名称", "value": "轻伤"}
{"action": "set", "key": "玩家角色状态.状态效果[0].描述", "value": "服用丹药后伤势好转"}
{"action": "set", "key": "玩家角色状态.状态效果[0].强度", "value": -3}
{"action": "set", "key": "玩家角色状态.状态效果[0].持续时间分钟", "value": 30}
\`\`\`

### ❌ 禁止的操作
**严禁使用 delete 或 unset 删除状态效果！**
\`\`\`json
// ❌ 错误示例
{"action": "delete", "key": "玩家角色状态.状态效果[0]"}
{"action": "unset", "key": "玩家角色状态.状态效果[0]"}
\`\`\`

## 常见场景
- **受伤后服药**：将"重伤"改为"轻伤"，降低强度，缩短持续时间
- **中毒后解毒**：将"剧毒"改为"余毒未清"，降低强度
- **自然恢复**：降低强度、缩短持续时间，不要直接删除

## 系统自动清理
系统会在每次对话后自动检查并移除过期的状态效果，AI只需正确设置持续时间即可。
`.trim();
