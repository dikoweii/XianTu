...
export function buildInitialMessagePrompt(userSelections: {
  ...
  return [
    ...
    '   - 位置必须包含完整的描述和坐标信息',
    '   - 位置生成格式：请使用“世界名 · 大州名 · 区域名 · 地点名”，不要只给城镇名；信息不足时使用“未知大州/未知区域”占位，但不得省略层级',
    '   - 描绘周围的景色、建筑、气氛等感官细节',
    '   - 结合出身背景创造合适的开场场景',
    '   - 描述当前的时间、天气、季节等背景要素',
    '',
    ...
    '## 📝 文本创作示例要求',
    '**环境描述示例风格**：',
    '"清晨的薄雾还未完全散去，我站在石砖铺就的庭院中，周围古朴的建筑在晨光中显得格外静谧。远山如黛，近水含烟，空气中弥漫着淡淡的桂花香气和晨露的清新。微风轻拂过脸颊，带来一丝凉意..."',
    '',
    '**人物互动示例风格**：',
...
export const INITIAL_MESSAGE_PROMPT = [
  ...
  '',
  '【NPC基础信息完整性要求】',
  '- **强制字段要求**：每个NPC的角色基础信息必须包含完整的性别、天资、出生、灵根字段',
  '- **属性具体化**：所有NPC的性别、天资、灵根都必须有具体内容，严禁使用"未知"，灵根必须为对象格式包含品级',
  '- **示例完整NPC结构**：',
  '  ```json',
  '  {',
  '    "角色基础信息": {',
  ...
  '  }',
  '  ```',
  '- **禁止使用占位符**：绝对禁止使用"未知"、"普通"、"出身背景"、"灵根类型"等占位符文本',
  '- **具体化要求**：必须生成具体的、有意义的值，如：',
  '  - 性别: "男"或"女"（具体性别）',
  '  - 天资: "凡夫俗子"、"中人之姿"、"天生灵秀"（具体天资名称）',
  '  - 出生: "世家子弟"、"平民百姓"、"山野村夫"（具体出身背景）',
  '  - 灵根: {"名称":"火灵根","品质":"中品","描述":"火属性灵根"}（完整对象格式，包含品质）',
  '- **数据验证**：AI生成的NPC数据将用于前端显示，缺失字段会显示为"未知"影响用户体验',
  '',
