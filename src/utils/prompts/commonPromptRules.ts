...
export const COMMON_NPC_RULES = [
  ...
  '- **🚨 行为模式生成（必需）**：',
  '  * **必须生成 `NPC行为` 字段**，包含 `行为模式` 和 `日常路线`。',
  '  * **行为模式**：必须从 "攻击", "防御", "中立", "逃跑", "未知" 中选择一个，默认为 "未知"。',
  '  * **日常路线**：必须生成至少1-3条符合其身份的日常活动路线，或明确设置为 `[]` 表示暂无。',
  '  * **示例**:',
  '    ```json',
  '    "NPC行为": {',
  '      "行为模式": "中立",',
  '      "日常路线": [',
  '        {"时间": "辰时", "位置": "宗门广场", "行为": "指导外门弟子"},',
  '        {"时间": "午时", "位置": "炼丹房", "行为": "炼制丹药"}',
  '      ]',
  '    }',
  '    ```',
  '',
  '- **🚨 背包生成要求（必需）**：',
  '  * **必须生成 `背包` 字段**，且必须同时包含 `灵石` 和 `物品` 两个对象。',
  '  * **灵石**：必须生成，四种品级（下品、中品、上品、极品）的灵石数量可为0，但结构必须存在。',
  '  * **物品**：根据NPC身份地位，携带1-5件合理物品。若无物品，`物品` 字段应为一个空对象 `{}`。',
  '  * **背包格式示例**：',
  '    ```json',
  '    "背包": {',
  '      "灵石": { "下品": 100, "中品": 10, "上品": 0, "极品": 0 },',
  '      "物品": {',
  '        "item_herb_01": { "名称": "清心草", "类型": "其他", "品质": {"quality": "凡", "grade": 1}, "数量": 10, "描述": "一种常见的草药。" },',
  '        "item_sword_02": { "名称": "铁剑", "类型": "装备", "品质": {"quality": "凡", "grade": 2}, "数量": 1, "描述": "一把普通的铁剑。", "已装备": false }',
  '      }',
  '    }',
  '    ```',
  '',
