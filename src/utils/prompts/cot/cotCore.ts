...
Result: [F] vs [Z] → 基础[失败/成功/大成功/完美] → 修正[1降/20升] → 最终[结果]
...
### 3. Writing Quality (质量检查)
**禁止**：❌ 机械化(过度分析/数据化思维/程序比喻) | ❌ 绝望化(空洞/失魂/麻木) | ❌ 八股化(石子入湖/惊雷/针刀) | ❌ 神化(无关境界的吹捧)

**必须**：✅ 生理细节(呼吸/心跳/体温/触感) | ✅ 环境描写(光影/声音/气味/温度) | ✅ 自然对话 | ✅ 真实情感
...
**字数规划**：简单500-800字 | 复杂1000-1500字 | 核心叙事≥60%

**叙事流程**：内心想法 → 动作描写 → 场景细节 → 氛围营造

**本次重点**：Focus: [战斗/对话/修炼/探索/情感] | Innovation: [场景特色/剧情转折]

### 6. Data & Commands (数据与指令)
**变更内容**：[列出本次会改变的：角色状态/物品/NPC/任务/时间/位置]

**校验清单**：
1. 路径：NPC(人物关系.{NPC名}.字段) | 玩家(玩家角色状态.字段) | 大道(三千大道.大道列表.{道名}.字段) | 物品(背包.物品.{ID}.字段)
...
2. 对象完整性：
   ...
   - 装备: {物品ID,名称,类型:"装备",品质,数量,描述,已装备}
   - 功法: {物品ID,名称,类型:"功法",品质,数量,描述,功法效果,功法技能[],修炼进度,已装备}
   - 丹药: {物品ID,名称,类型:"丹药",品质,数量,描述,使用效果}
   - NPC: {姓名,性别,年龄,种族,外貌描述,性格特征[],境界,灵根,天赋[],先天六司,与玩家关系,好感度,记忆[],当前位置,当前内心想法}
   - 任务: {任务ID,任务名称,任务描述,任务类型,任务状态,目标列表[],奖励}

3. 修行消耗：使用技能/法术 → 扣除灵气/神识 + 增加使用次数 | 资源不足时叙事说明
