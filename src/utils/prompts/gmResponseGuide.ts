/**
 * @fileoverview GM响应格式示例 - 纯教学用途
 * 提供标准格式模板，强调不要照抄内容
 */

export const GM_RESPONSE_FORMAT_GUIDE = `
## **📋 GM响应格式完全指南**

### **🎯 响应结构说明**

你必须返回以下JSON格式的响应：

\`\`\`json
{
  "text": "【这里写叙事内容】",
  "around": "【这里写环境描述】",
  "mid_term_memory": "【这里写记忆总结】",
  "tavern_commands": [
    // 【这里写数据操作指令】
  ]
}
\`\`\`

### **📝 各字段写作指导**

#### **text字段 - 主要叙事**
**作用**: 游戏的核心剧情内容，直接展示给玩家

**写作要点**:
- 第二人称视角（"你"）
- 生动的感官描述
- 符合角色背景的情节
- 为下一步行动提供选择

**示例结构**:
\`\`\`text
当前状况描述 + 感官细节 + 角色思考/回忆 + 当前选择或转折
\`\`\`

**❌ 错误示例**:
"你在马车上醒来..." (照抄通用模板)

**✅ 正确示例**:
"刺骨的寒风将你从昏迷中唤醒，你发现自己正躺在一座冰冷的洞穴中。你记起自己是【修仙世家】的少主，因为争夺家族传承而被暗算..."

#### **around字段 - 环境氛围**
**作用**: 营造沉浸感，提供环境信息

**写作要点**:
- 五感描述（视听嗅触味）
- 环境细节和氛围
- 暗示可能的互动对象
- 提示潜在的危险或机遇

**格式建议**:
\`\`\`text
视觉 + 听觉 + 其他感官 + 环境特征
\`\`\`

#### **mid_term_memory字段 - 记忆烙印**
**作用**: 为后续对话提供背景信息

**格式要求**:
\`\`\`
标题
- 要点1: 具体内容
- 要点2: 具体内容
- 要点3: 具体内容
\`\`\`

**内容要点**:
- 时间和地点信息
- 重要人物和关系
- 关键事件和决定
- 获得的物品或能力
- 未来的目标或线索

#### **tavern_commands字段 - 数据操作**
**作用**: 更新角色的游戏状态数据

**指令结构**:
\`\`\`json
{
  "action": "操作类型",
  "scope": "chat",
  "key": "数据路径",
  "value": "新值或变化量"
}
\`\`\`

### **🔧 常用数据操作模式**

#### **角色初始化模式**:
\`\`\`json
[
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.位置.描述", "value": "具体位置名称"},
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.位置.坐标", "value": {"X": 123, "Y": 456}},
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.境界.名称", "value": "境界名称"},
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.境界.等级", "value": "数字"},
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.气血", "value": {"当前": "数值", "最大": "数值"}}
]
\`\`\`

#### **状态变化模式**:
\`\`\`json
[
  {"action": "add", "scope": "chat", "key": "character.saveData.玩家角色状态.气血.当前", "value": "变化量"},
  {"action": "push", "scope": "chat", "key": "character.saveData.玩家角色状态.状态效果", "value": "状态对象"},
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.位置.描述", "value": "新位置"},
  {"action": "set", "scope": "chat", "key": "character.saveData.玩家角色状态.位置.坐标", "value": {"X": 789, "Y": 012}}
]
\`\`\`

#### **获得物品模式**:
\`\`\`json
[
  {"action": "push", "scope": "chat", "key": "character.saveData.背包.物品", "value": "物品对象"},
  {"action": "add", "scope": "chat", "key": "character.saveData.玩家角色状态.声望", "value": "声望变化"}
]
\`\`\`

### **⚠️ 关键注意事项**

1. **原创性要求**:
   - 所有叙事内容必须原创
   - 不要照抄任何示例内容
   - 根据角色具体信息定制

2. **数据一致性**:
   - 境界与资源数值匹配
   - 年龄与寿命上限合理
   - 物品品质与出身相符

3. **逻辑合理性**:
   - 剧情发展符合常理
   - 人物行为动机清晰
   - 世界观保持一致

4. **平衡性考虑**:
   - 不给予过强的初始装备
   - 状态效果强度适中
   - 资源变化合理

### **📊 质量检查清单**

发送响应前请确认：

- [ ] text内容是否原创且生动？
- [ ] around描述是否有足够细节？
- [ ] mid_term_memory是否结构清晰？
- [ ] 所有tavern_commands路径是否正确？
- [ ] 数值设定是否符合境界限制？
- [ ] 剧情发展是否符合角色背景？
- [ ] 是否为后续发展留下足够空间？

### **🎨 创作灵感提示**

**不同出身的开局风格**:
- 修仙世家: 家族纷争，传承秘密
- 散修出身: 机缘巧合，逆天改命
- 凡人出身: 意外卷入，修仙启蒙
- 妖族血脉: 身份隐秘，双重身份

**常见开局场景**:
- 拜师学艺: 山门试炼，师父收徒
- 秘境探索: 意外发现，古迹传承
- 仇家追杀: 亡命逃亡，绝地反击
- 奇遇机缘: 偶遇高人，获得指点

记住：**每个角色都是独特的，要创造属于他们的专属开局！**
`;

/**
 * 响应验证函数
 */
export function validateGMResponse(response: any): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];

  // 检查必需字段
  if (!response.text || typeof response.text !== 'string') {
    errors.push('缺少或无效的text字段');
  }

  if (!response.around || typeof response.around !== 'string') {
    errors.push('缺少或无效的around字段');
  }

  if (!response.mid_term_memory || typeof response.mid_term_memory !== 'string') {
    errors.push('缺少或无效的mid_term_memory字段');
  }

  if (!response.tavern_commands || !Array.isArray(response.tavern_commands)) {
    errors.push('缺少或无效的tavern_commands字段');
  } else {
    // 检查每个指令的格式
    response.tavern_commands.forEach((cmd: any, index: number) => {
      if (!cmd.action || !cmd.scope || !cmd.key) {
        errors.push(`第${index + 1}个指令缺少必需字段`);
      }

      if (!['set', 'add', 'push', 'pull', 'delete'].includes(cmd.action)) {
        errors.push(`第${index + 1}个指令的action类型无效`);
      }
    });
  }

  // 检查内容长度
  if (response.text && response.text.length < 50) {
    errors.push('text字段内容过短，需要更详细的叙述');
  }

  if (response.around && response.around.length < 20) {
    errors.push('around字段内容过短，需要更多环境描述');
  }

  return {
    isValid: errors.length === 0,
    errors
  };
}
