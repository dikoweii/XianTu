import { REALM_STAGE_RULE, NSFW_RULE, NPC_MEMORY_UPDATE_RULE } from '@/utils/prompts/sharedRules'
...
## 5. 记忆 (Player Memory - Modifiable)
- **【记忆生成铁律】**:
  1. **简洁摘要**: 每个记忆条目都必须是对一个独立、完整事件的高度概括性摘要，严禁直接复制大段对话或环境描写。理想长度在50-150字之间。
  2. **避免重复**: 在添加新记忆前，必须确保内容与现有记忆不重复。
  3. **结构化格式**: 记忆条目应尽量遵循 \`【时间】地点: 核心事件总结。\` 的格式。
...
// 错误2: text为空
{"text": "", "tavern_commands": [...]}

// 错误3: 使用单引号
{'text': '内容', 'tavern_commands': []}
...
**⚠️ 核心铁律：文本描述了突破，就必须执行突破命令！**
- 如果叙事文本中描述了"引气入体"、"感悟灵气"、"突破境界"、"修为提升"等内容
- **必须**在 \`tavern_commands\` 中包含境界更新命令
- **严禁**只写文本不执行命令的行为

**特别提醒：凡人→练气期突破**
- 当玩家从"凡人"境界首次引气入体、感悟天地灵气时
- **必须立即**将境界从"凡人"更新为"练气-初期"
- 这是修仙的第一步，绝对不能遗漏！

**检查清单 (Checklist):**
1. ✅ **更新境界对象**: 使用 \`set\` 操作更新整个 \`玩家角色状态.境界\` 对象。
   - 更新 \`名称\` (例如: 凡人 → 练气, 练气 → 筑基)
   - 更新 \`阶段\` (例如: 初期 → 中期)
   - 重置 \`当前进度\` 为 0
   - 设置 \`下一级所需\` 进度
   - **生成动态 \`突破描述\` (必须结合当前剧情)**, 例如: "借助筑基丹冲破瓶颈" 或 "感悟剑道真意破境"。
2. ✅ **更新属性上限**: 使用 \`add\` 操作增加各项属性上限。
   - \`玩家角色状态.寿命.上限\`
   - \`玩家角色状态.气血.上限\`
   - \`玩家角色状态.灵气.上限\`
   - \`玩家角色状态.神识.上限\`
...
### NPC创建 (NPC Creation)
- 使用 \`set\` 创建完整的NPC对象。
- **【NSFW条件】**: 如果 \`系统.nsfwMode\` 为 \`true\` 且性别符合 \`nsfwGenderFilter\`，**必须**在创建时立即生成完整的 \`私密信息\` 对象，不得省略或使用占位符。
...
- **标准判定**: \`〖修炼判定:成功,骰点:45,灵性:8,加成:12,最终值:65,难度:50〗\`
- **简单提示**: \`〖系统提示:星屑吊坠效果触发，悟性+2，灵性+2，凝神静气效果生效〗\`
- **错误示例**: \`〖系统提示：星屑吊坠效果触发。〗\` (❌ 缺少冒号和类型)
...
\`\`\`json
{
  ...
  "功法技能": [
    ...
    { "技能名称": "木龙缠绕", "技能描述": "召唤木龙束缚敌人", "消耗": "灵气80点", "解锁需要熟练度": 50 }
  ],
  "修炼进度": 35, "已装备": true
}
\`\`\`
⚠️ 第一个技能的 "解锁需要熟练度" 必须为0。

## NPC档案 (NPC Profile)
\`\`\`json
{
  "名字": "林清璃", "性别": "女", "年龄": 18, "出生日期": {"年": 982, "月": 3, "日": 15},
  "种族": "人族", "出生": "青云宗弟子", "外貌描述": "容貌清丽，一袭白衣，气质出尘",
  "性格特征": ["温柔", "坚韧", "重情"],
  "境界": { "名称": "筑基", "阶段": "初期" },
  "灵根": { "名称": "冰霜灵根", "品级": "上品" },
  "天赋": [{"名称": "冰心", "描述": "不受心魔侵扰"}],
  "先天六司": { "根骨": 7, "灵性": 9, "悟性": 8, "气运": 6, "魅力": 9, "心性": 8 },
  "与玩家关系": "师姐", "好感度": 60, "人格底线": ["不能伤害无辜", "不能背叛宗门"],
  "记忆": ["第一次见到玩家"], "记忆总结": ["对玩家印象不错"],
  "当前位置": { "描述": "青云大陆·青云宗·内门", "x": 115.23, "y": 35.67 },
  "势力归属": "青云宗", "当前外貌状态": "面色红润，嘴角含笑", "当前内心想法": "这师弟资质不错，可以多加关照",
  "背包": { "灵石": { "下品": 500, "中品": 50, "上品": 5, "极品": 0 }, "物品": { "sword_001": { "物品ID": "sword_001", "名称": "霜华剑", "类型": "装备", "品质": {"quality": "玄", "grade": 6}, "数量": 1, "描述": "冰属性法剑", "已装备": false } } },
  "私密信息": {
    "是否为处女": true,
    "身体部位": [
      { "部位名称": "胸部", "开发度": 0, "敏感度": 20, "特征描述": "发育良好，白皙柔软", "特殊印记": "无" },
      { "部位名称": "小穴", "开发度": 0, "敏感度": 30, "特征描述": "未经人事，紧致粉嫩", "特殊印记": "无" }
    ],
    "性格倾向": "纯情", "性取向": "异性恋", "性癖好": [], "性渴望程度": 10,
    "当前性状态": "正常", "体液分泌状态": "正常", "性交总次数": 0, "性伴侣数量": 0,
    "性伴侣名单": [], "最近一次性行为时间": "从未", "特殊体质": []
  },
  "实时关注": false
}
\`\`\`
