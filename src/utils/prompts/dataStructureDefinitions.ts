...
\`\`\`json
{
  "名称": "string (如：凡人, 练气, 筑基)",
  "阶段": "string (如：初期, 中期, 后期, 圆满)",
  "当前进度": "number (当前阶段的经验值)",
  "下一级所需": "number (达到下一阶段所需的总经验值)"
}
\`\`\`
...
**宗门操作规则**：
1. 加入宗门：必须检查 \`世界信息.势力信息\` 中的 \`canJoin\` 和 \`joinRequirements\`
2. 宗门职位提升：需要足够的贡献值和境界
3. 宗门任务：通过tavern_commands更新贡献值
4. 叛离宗门：会影响与该宗门的关系，设为"仇敌"或"敌对"
...
**装备流程（三步法）**：
1. **创建物品**：\`set "背包_物品.<物品ID>" = {完整物品对象}\`
2. **装备引用**：\`set "装备栏.装备N" = "<物品ID>"\`
3. **标记已装备**：\`set "背包_物品.<物品ID>.已装备" = true\`

**卸下装备流程**：
1. \`set "装备栏.装备N" = null\`
2. \`set "背包_物品.<物品ID>.已装备" = false\`

### 修炼功法系统 (Cultivation Technique)
...
4.  **功法效果**：
    *   **修炼速度加成**：**必须**提供数值格式，1.35表示35%加成。品质越高加成越大：凡品1.05-1.15，黄品1.2-1.35，玄品1.4-1.6，地品1.7-1.9，天品2.0-2.3，仙品2.5-3.0，神品3.5-5.0
    *   **属性加成**：品质越高加成越多。高品质功法应有多个属性加成。
    *   **特殊能力**：玄品及以上应添加特殊效果。
5.  **功法技能**（⚠️ 重要）：
    *   **凡品/黄品**：可选，可以没有技能
    *   **玄品**：**必须**至少1个技能
