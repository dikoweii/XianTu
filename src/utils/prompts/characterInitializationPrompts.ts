...
// 角色初始化专用规则 (精简优化版 - 从172行缩减至45行)
const CHARACTER_INIT_RULES = [
  '# 🎯 核心任务：生成开局故事+初始化世界状态',
  '',
  '## 1️⃣ 随机选项处理（强制）',
  '**检测"随机灵根"或"随机出身"字符串时必须替换**：',
  '- 灵根：根据天资选品级(凡人→凡/下品,俊杰→中/上品,天骄→极品,妖孽→神品),创造独特名称',
  '- 出身：根据世界背景选择合理出身,影响初始资源',
  '⚠️ text和commands中严禁出现"随机"字样',
  '',
  '## 2️⃣ 玩家选择不可修改',
  '如玩家已选具体灵根/出身(非"随机"),严禁用commands修改`基础信息.灵根`或`基础信息.出生`',
  '',
  '## 3️⃣ 位置设置规则',
  '**第一层必须用availableContinents数组中的真实大陆名**：',
  '- 格式: "真实大陆名·地点名" 或 "真实大陆名·区域名·地点名"',
  '- 优先从availableLocations选地点',
  '- 示例: `{"action":"set","key":"位置.描述","value":"青霄大陆·西境·幽冥绝地"}`',
  '⚠️ 严禁编造不存在的大陆名(如"南荒""东胜神州")',
  '',
  '## 4️⃣ 物品/功法创建流程',
  '**必须先创建到背包,再装备/修炼**：',
  '```json',
  '// 步骤1: 创建到背包(背包_物品是对象非数组)',
  '{"action":"set","key":"背包_物品.sword_001","value":{物品ID,名称,类型,品质,描述...}}',
  '// 步骤2: 设置装备栏引用',
  '{"action":"set","key":"装备栏.装备1","value":{物品ID:"sword_001",名称:"玄铁剑"}}',
  '// 步骤3: 标记已装备',
  '{"action":"set","key":"背包_物品.sword_001.已装备","value":true}',
  '```',
  '',
  '## 5️⃣ 开局年龄不可修改',
  '从玩家选择的`开局年龄`精确开始叙事,严禁通过commands修改`属性.寿命.当前`,严禁时间跳跃',
  '',
  '## 6️⃣ 初始资源配置',
  '- **灵石**：根据出身(凡人0-10,小康10-50,宗门弟子50-200下品灵石)',
  '- **物品**：2-5件(生活用品+特殊物品),品质凡人凡品,小宗门弟子黄品,大宗门弟子玄品',
  '- **NPC**：0-3个(师父/同门/朋友),使用简化格式',
  '- **功法**：0-2部,品级匹配出身',
  '',
  '## 7️⃣ 叙事要求（精简）',
  '- **字数**：1200-2500字沉浸式开场(降低要求减少重试)',
  '- **内容**：展现出身/天赋/环境,用行为暗示特质(避免直接提"根骨8""神品灵根"等术语)',
  '- **格式**：纯叙事内容,不加时间标记(系统自动添加)',
  '- **风格**：体现修仙世界残酷性(修炼艰难,资源稀缺,风险高)',
  '',
  REALM_STAGE_RULE,
  '',
  NSFW_RULE,
...
\`\`\`json
{
  ...
  "tavern_commands": [
    {"action": "set", "key": "位置.描述", "value": "真实大陆名·地点名"},
    {"action": "set", "key": "背包_物品.<物品ID>", "value": {完整物品对象}},
    {"action": "set", "key": "系统任务", "value": {"配置":{"启用":false,"任务类型":"all","颁发数量":3,"自定义提示词":""},"进行中任务":[],"已完成任务名称":[]}},
    更多初始化指令...
  ]
}
\`\`\`

**JSON要求**：禁用注释、尾随逗号、单引号；text字段必需且≥1200字；tavern_commands必需

---

# 1. 开局故事 (text)

*   核心: 生成一段 1200-2500 字的沉浸式开场故事。
*   内容: 故事必须体现玩家选择的出身、天赋，并详细描绘角色当前所处的环境、心境和正在发生的事件。
*   **叙事手法**：用行为、感受、环境互动和他人反应展现角色特质,严禁直接提及游戏术语(如"天资""灵根""根骨:8")。
    *   高"根骨"→描述体魄强健、气血充盈
    *   高"悟性"→描述一点就通、领悟力超群
    *   "神品灵根"→描述天地异象、对灵气的恐怖亲和力
*   **叙事一致性**：故事必须与玩家选择的所有核心设定(天资/出身/灵根/天赋)完全一致,积极体现这些设定的优势。
*   **开局年龄**：必须从玩家选择的【开局年龄】精确开始,严禁修改【属性.寿命.当前】,严禁时间跳跃。
*   **格式**：纯叙事内容,不加时间标记(系统自动添加)。
...
*   内容（初始化要求）：
    *   **物品与灵石**: 必须在背包中添加2-5件符合背景的初始物品,并根据出身设定初始灵石。
        *   灵石:根据出身(凡人0-10,小康10-50,宗门弟子50-200下品灵石),严禁设置为0除非赤贫开局。
        *   基础生活用品(如衣物、食物等)
        *   根据出身的特殊物品(传家之宝、修炼资源等)
        *   品质限制:凡人出身凡品,小宗门弟子黄品封顶,大宗门弟子玄品,特殊开局可能地/天/仙品
    *   **NPC**: 必须创建0-3个与主角相关的初始NPC,使用简化格式(师父/同门/朋友/孤独一人)。
    *   **大道**: 如果天赋对应,解锁初始大道。
