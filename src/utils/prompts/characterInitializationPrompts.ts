...
const CHARACTER_INIT_RULES = [
  ...
  '## 随机灵根和随机出生处理',
  '1. **随机灵根必须替换**：',
  '   - 如果玩家选择了"随机灵根"，你**必须**为其随机生成一个具体的灵根',
  '   - 随机确定品级（凡品、下品、中品、上品、极品、天品、神品）',
  '   - 生成完整的灵根对象：{名称: "具体灵根名", 品级: "具体品级", 描述: "详细描述"}',
  '   - **严禁**在最终结果中保留"随机灵根"字样',
  '',
  '2. **随机出生必须替换**：',
  '   - 如果玩家选择了"随机出生"，你**必须**为其随机生成一个具体的出身背景',
  '   - 从可用的出身类型中随机选择（山野遗孤、书香门第、商贾之子等）',
  '   - 生成符合世界观的具体出身描述',
  '   - **严禁**在最终结果中保留"随机出生"字样',
  '',
  '3. **随机化原则**：',
  ...
  '- 必须生成具体的地理位置（大陆名·地点名）',
  '- 创建0-5件初始物品（衣服、武器、丹药等），并根据出身设定初始灵石。',
  '- 创建0-2个初始功法（基础心法或高级功法，取决于出身）',
...
\`\`\`json
{
  "text": "⚠️ 必须1000-1500字的详细开局故事，包含环境描写、人物刻画、事件叙述等",
  "mid_term_memory": "可选：开局核心事件50-100字总结",
  "tavern_commands": [
    {"action": "set", "key": "玩家角色状态.位置.描述", "value": "大陆名·地点名（⚠️ 大陆名必须从 availableContinents 中选择，地点名必须从 availableLocations 中选择，严禁编造）"},
    {"action": "set", "key": "背包_物品.<物品ID>", "value": {完整物品对象}},
    {"action": "set", "key": "人物关系.<NPC名称>", "value": {完整NPC对象}},
    更多初始化指令...
  ]
}
\`\`\`
...
**🚨 关键提醒：位置名称验证（极其重要）**
在设置 \`玩家角色状态.位置.描述\` 时，**必须严格遵守以下规则**：
1. **大陆名称**：必须从顶层的 \`availableContinents\` 数组中选择一个真实的大陆名称
2. **地点名称**：必须从顶层的 \`availableLocations\` 数组中选择一个真实的地点名称
3. **严禁编造**：严禁编造任何不存在于输入数据中的大陆名或地点名
4. **格式**："大陆名·地点名"（用一个·分隔）

**示例**（假设输入数据中有这些真实数据）：
- 如果 availableContinents = [{名称: "朝天大陆"}]，availableLocations = [{名称: "青云门"}, {名称: "天枢城"}]
- ✅ 正确："朝天大陆·青云门" 或 "朝天大陆·天枢城"（都真实存在）
- ❌ 错误："朝天大陆·青石镇"（青石镇不在地点列表中，是编造的）
- ❌ 错误："南荒·青云门"（南荒不在大陆列表中，是编造的）
...
*   **⚠️ 随机选项处理要求**：
    *   如果检测到"随机灵根"，必须在tavern_commands中设置具体的灵根对象，格式：
        \`\`\`
        {"action": "set", "key": "角色基础信息.灵根", "value": {"名称": "具体灵根名", "品级": "具体品级", "描述": "详细描述"}}
        \`\`\`
    *   如果检测到"随机出生"，必须在tavern_commands中设置具体的出身描述，格式：
        \`\`\`
        {"action": "set", "key": "角色基础信息.出生", "value": "具体出身名称"}
        \`\`\`
    *   **严禁**在任何tavern_commands中保留"随机"字样。
*   内容:
    *   **位置设置规范（重要）**：
        *   ⚠️ **强制要求**：必须从输入数据的 \`availableContinents\` 数组中选择一个大陆名称
        *   **严禁编造大陆名**：不要使用"南荒"、"北域"等未在 \`availableContinents\` 中出现的大陆名
        *   **严禁添加中间层级**：不要在大陆名和地点名之间添加"北境"、"南域"等区域名
        *   **位置格式**："大陆名称·地点名"（只用一个·分隔，两个部分）
        *   **正确示例**：
            - ✅ "朝天大陆·青云门"
            - ✅ "碎云神州·落雁城"
        *   **错误示例**：
...
5. **示例**：
   - **修仙世界**
     - 输入: \`{ "continent_name": "朝天大陆" }\`
     - 输出: \`{ "location_name": "青云宗·外门·讲武堂" }\`
   - **现代都市**
     - 输入: \`{ "continent_name": "地球" }\`
     - 输出: \`{ "location_name": "江城大学·图书馆·三楼自习室" }\`
