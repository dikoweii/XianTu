...
const CHARACTER_INIT_RULES = [
  ...
  '',
  '### NSFW模式',
  '当nsfwMode=true时，需为玩家和NPC生成身体部位开发数据。',
...
**核心原则**：
- **严格遵循玩家选择**：根据用户选择的出生背景等信息生成对应的开局
- **尊重玩家意图**：玩家可以选择任何生活方式，不强制修仙或冒险
- **开放性叙事**：这个含有修仙体系，但玩家可以选择成为修士、凡人、商人、学者等任何角色
...
- ✅ **位置设置** (第一个命令): {"action":"set","key":"玩家角色状态.位置.描述","value":"世界名·大陆名·地点名"}
...
- **资产**: \`背包\` ({灵石, 物品})
- **私密信息**: (当 nsfwMode=true 时) 包含14个必填字段的完整对象
- **元信息**: \`实时关注\` (boolean)
