...
export class EnhancedWorldPromptBuilder {
  static buildPrompt(config: WorldPromptConfig): string {
    ...
    const cities = Math.max(1, Math.floor(remainingLocations * 0.3)); // 保证至少有城镇
    const specialSites = Math.max(1, Math.floor(remainingLocations * 0.25)); 
    const dangerZones = Math.max(1, Math.floor(remainingLocations * 0.15)); 
...
- **边界设计**: 大洲边界用简单规则多边形表示（6-8个坐标点），避免过于复杂的形状
...
- ✓ **势力范围**: ⚠️**严禁空数组**⚠️ 必须是坐标点数组，至少4个点形成封闭多边形，每个势力都必须有势力范围！**尺寸要求：势力范围应占大洲面积的8%-25%，跨度0.8-2.5经纬度**
...
- **具体尺寸对比**: 
  * **大洲边界**: 跨度10-15经纬度，覆盖地图的主要部分，**严禁重叠**
  * **势力范围**: 跨度0.8-2.5经纬度，是大洲内的较大控制区域
- **地理逻辑**: 
  ...
  "continents": [
    {
      ...
      "大洲边界": [
        ...
        {"longitude": ${minLng + Math.round(((maxLng - minLng) / 2) * 10) / 10}, "latitude": ${minLat}},
        {"longitude": ${minLng + Math.round(((maxLng - minLng) / 2) * 10) / 10}, "latitude": ${maxLat}},
        {"longitude": ${minLng}, "latitude": ${maxLat}}
      ],
      "主要势力": ["在此大洲的主要势力ID列表"]
    }
  ],
  "factions": [
    {
      ...
      "势力范围": [
        ...
        // ⚠️ 重要：势力范围可以比较大，占大洲面积的8%-25%
        // 示例：如果大洲跨度10-15经纬度，势力范围应该有0.8-2.5经纬度跨度
        {"longitude": ${minLng + 2.0}, "latitude": ${minLat + 1.0}},
        {"longitude": ${minLng + 4.5}, "latitude": ${minLat + 1.2}},
        {"longitude": ${minLng + 4.8}, "latitude": ${minLat + 3.0}},
        {"longitude": ${minLng + 2.2}, "latitude": ${minLat + 3.5}},
        {"longitude": ${minLng + 1.8}, "latitude": ${minLat + 2.0}}
      ],
      "leadership": {
        "宗主": "具体姓名（必填，如：欧阳烈风）",
        "宗主修为": "具体境界（必填，如：化神中期、元婴后期等）",
