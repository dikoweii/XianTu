...
@router.get('/by-username/{username}', response_model=schema.PlayerAccount, tags=["用户"])
...
async def get_user(user_id: int, current_admin: AdminAccount = Depends(deps.get_admin_or_super_admin)):
    """
    根据ID获取用户信息（管理员权限）
    """
